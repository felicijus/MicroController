...
#define NEC_TRAILER_MARK 560
  ...
  sigTime = Timer2.micros_10(); //keeps rolling track of signal time to avoid impact of loop & code execution delays
...
void SendIR::mark(unsigned int mLen) { //uses sigTime as end parameter
  ...
  unsigned long now = Timer2.micros_10();
  unsigned long dur = sigTime - now; //allows for rolling time adjustment due to code execution delays
  if (dur == 0) return;
  while ((Timer2.micros_10() - now) < dur) { //just wait here until time is up
